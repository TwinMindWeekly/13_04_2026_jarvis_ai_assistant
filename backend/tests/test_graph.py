...
def test_folder_of_root_file():
    assert _folder_of("file.pdf") == ""


def test_folder_of_nested_file():
    assert _folder_of("subdir/file.pdf") == "subdir"
...
def test_cache_roundtrip(tmp_path, monkeypatch):
    ...
    data = GraphData(
        nodes=[],
        links=[],
        meta=GraphMeta(total_docs=0, total_links=0, threshold=0.5),
    )
    key = "abc123"
    save_cache(key, data)
    loaded = load_cache(key)
    assert loaded is not None
    assert loaded.meta.threshold == 0.5
...
def test_build_graph_empty_metadata(tmp_path, monkeypatch):
    ...
    data = asyncio.run(build_document_graph(threshold=0.5))
