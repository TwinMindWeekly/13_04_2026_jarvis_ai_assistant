...
def _word_save_as(path: str) -> str:
    app = _word_app()
    if app.Documents.Count == 0:
...
def _excel_save_as(path: str) -> str:
    app = _excel_app()
    if app.Workbooks.Count == 0:
...
def _powerpoint_save_as(path: str) -> str:
    app = _powerpoint_app()
...
_ACTIONS: dict[str, tuple[Any, list[str]]] = {
...
class OfficeAutomationTool(BaseTool):
    ...
    description = (
        ...
        "Deterministic alternative to clicking through the UI — use this "
        "instead of desktop_control whenever the action can be expressed as "
        "a COM call. Actions: "
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        handler, required = _ACTIONS[action]
